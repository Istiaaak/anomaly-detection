...
if __name__ == "__main__":
    ...
    # Modification de dashboard_realtime pour utiliser Plotly
    def dashboard_realtime(file_paths):
        """
        Prend une liste de chemins de fichiers image (upload via gr.Files avec type="filepath")
        et calcule :
          - Un histogramme des scores d'anomalie
          - Une tendance (graphique en ligne) des scores
          - Quelques statistiques (moyenne, écart-type, min, max)
        Retourne un résumé textuel et un objet Plotly affichant le dashboard.
        """
        if not file_paths:
            return "Aucune image uploadée.", None
        ...
        fig = make_subplots(rows=1, cols=2, 
                            subplot_titles=("Histogramme des Scores d'Anomalie", "Tendance des Scores d'Anomalie"))

        # Histogramme
        fig.add_trace(
            go.Histogram(x=anomaly_scores, nbinsx=20, marker_color="blue", opacity=0.7, name="Histogramme"),
            row=1, col=1
        )

        # Courbe de tendance (ligne)
        fig.add_trace(
            go.Scatter(x=list(range(len(anomaly_scores))), y=anomaly_scores,
                       mode="lines+markers", line=dict(color="green"), name="Tendance"),
            row=1, col=2
        )
        
        # Mise à jour de la mise en page globale avec un titre contenant les statistiques
        fig.update_layout(
            title_text=f"Moyenne: {mean_score:.3f}, Écart-type: {std_score:.3f}, Min: {min_score:.3f}, Max: {max_score:.3f}",
            showlegend=False,
            width=1000,
            height=500
        )
    ...
    # 3. Dashboard Temps Réel (sans ground truth) avec Plotly
    iface_realtime_dashboard = gr.Interface(
        fn=dashboard_realtime,
        inputs=gr.Files(type="filepath", label="Images test (multi-upload)"),
        outputs=[gr.Textbox(label="Résumé des métriques"), gr.Plot(label="Dashboard")],
