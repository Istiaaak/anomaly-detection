...
class ViTWrapper(VisionTransformerWrapper):
    ...
    def prepare_image(self, img):
        ...
        # ajout d'une dimension pour le batch (1, C, H, W)
    ...
    def extract_features(self, img_tensor):
        with torch.no_grad():
            ...
            # découpe l'image en plusieurs patches
            # prend en entrée (batch_size, channel, height, weight)
            # renvoie en sortie (batch_size, nbr_patches, hidden_dim)
            ...
            return patch_features[:, 1:, :].squeeze().cpu().numpy()  
